...
Namespace Texte
    Friend Class ObjectPool(Of T As {IPoolableType(Of T), New})
        Private ReadOnly pool As Queue(Of T) = New Queue(Of T)()
    ...
    Friend Class TexturePacker(Of T As IPackable(Of T))
        ...
        Friend Sub New(width As Integer, height As Integer)
            If width <= 0 Then Throw New ArgumentOutOfRangeException(NameOf(width), width, "Must be greater than zero.")
            If height <= 0 Then Throw New ArgumentOutOfRangeException(NameOf(height), height, "Must be greater than zero.")
...
#End Region
    ...
    Public NotInheritable Class TexteGL
...
#Region "Fields"
        ...
        Private ReadOnly ContextCreation As IGraphicsContext
#End Region
#Region "Constructors"
        ...
        Public Sub New(quality As TextQuality, ContextCreation As IGraphicsContext)
            Me.New(Nothing, Nothing, quality, ContextCreation)
        End Sub

        Private Sub New(rasterizer As IGlyphRasterizer, output As ITextOutputProvider, quality As TextQuality, ContextCreation As IGraphicsContext)
            glyph_rasterizer = rasterizer
            text_output = output
            text_quality = quality
            Me.ContextCreation = ContextCreation
        End Sub
#End Region
...
#Region "Print"
        ...
        Public Sub Ecrire(text As String, font As Font, color As Color) Implements ITextPrinter.Ecrire
        ...
        Public Sub Ecrire(text As String, font As Font, color As Color, rect As RectangleF) Implements ITextPrinter.Ecrire
        ...
        Public Sub Ecrire(text As String, font As Font, color As Color, rect As RectangleF, options As TextPrinterOptions) Implements ITextPrinter.Ecrire
        ...
        Public Sub Ecrire(text As String, font As Font, color As Color, rect As RectangleF, options As TextPrinterOptions, alignment As TextAlignment) Implements ITextPrinter.Ecrire
        ...
        Public Sub Ecrire(text As String, font As Font, color As Color, rect As RectangleF, options As TextPrinterOptions,
...
#Region "Measure"
        ...
        Public Function Mesurer(text As String, font As Font) As TextExtents Implements ITextPrinter.Mesurer
        ...
        ''' <param name="rect">The System.Drawing.Rectangle that defines the bounds for text layout.</param>
        ''' <returns>An OpenTK.Graphics.TextExtents instance that contains the results of the measurement.</returns>
        Public Function Mesurer(text As String, font As Font, rect As RectangleF) As TextExtents Implements ITextPrinter.Mesurer
        ...
        ''' <returns>An OpenTK.Graphics.TextExtents instance that contains the results of the measurement.</returns>
        Public Function Mesurer(text As String, font As Font, rect As RectangleF, options As TextPrinterOptions) As TextExtents Implements ITextPrinter.Mesurer
        ...
        ''' <returns>An OpenTK.Graphics.TextExtents instance that contains the results of the measurement.</returns>
        Public Function Mesurer(text As String, font As Font, rect As RectangleF, options As TextPrinterOptions,
...
#Region "Private Members"
        Friend Shared ReadOnly Property CurrentContext As IGraphicsContext
        ...
        End Property
#End Region
        Private Sub VerifierContexte()
            If Not ContextCreation.IsCurrent Then
                Throw New Exception("Le contexte OpenGL courent n'est pas le contexte de création")
            End If
            If _CurrentContext IsNot ContextCreation Then _CurrentContext = ContextCreation
        End Sub
        Private Shared Function ValidateParameters(text As String, font As Font, rect As RectangleF) As Boolean
            ...
            If rect.Width < 0 OrElse rect.Height < 0 Then Throw New ArgumentOutOfRangeException(NameOf(rect))
            Return True
        End Function
...
#Region "Fields"
        ...
        Protected glyph_extents As List(Of RectangleF)
        Public Shared ReadOnly Empty As TextExtents = New TextExtents()
#End Region
        Friend Sub New()
            glyph_extents = New List(Of RectangleF)()
        End Sub
#Region "Public Members"
        ...
        Default Public Property Item(i As Integer) As RectangleF
            Get
        ...
        End Property
        ''' <summary> Gets the extents of each glyph in the measured text. </summary>
        Public ReadOnly Property GlyphExtents As IEnumerable(Of RectangleF)
            Get
...
#Region "Public Members"
        ...
        Friend Sub WriteRegion(source As Rectangle, target As Rectangle, mipLevel As Integer, bitmap As Bitmap)
            If bitmap Is Nothing Then Throw New ArgumentNullException(NameOf(bitmap)) '"data")
        ...
        Friend Shared Function ReadRegion(rect As Rectangle, mipLevel As Integer) As TextureRegion2D
            If mipLevel < 0 Then Throw New ArgumentOutOfRangeException(NameOf(mipLevel)) '"miplevel")
...
#Region "Private Members"
        Private Function CreateTexture() As Integer
            ...
            GL.TexImage2D(TextureTarget.Texture2D, 0, InternalFormat, Width, Height, 0, OpenGL.PixelFormat.Rgba, PixelType.UnsignedByte, IntPtr.Zero)
            Return id
        End Function

        Private Shared Sub SetDefaultTextureParameters(id As Integer)
...
#Region "IGraphicsResource Members"
        ...
        Private ReadOnly Property Id As Integer Implements IGraphicsResource.Id
            Get
                If idField = 0 Then
                    contextField = TexteGL.CurrentContext
                    idField = CreateTexture()
                End If
...
#End Region
    ...
    Friend Class TextureFont
        ...
        Private ReadOnly loaded_glyphs As Dictionary(Of Char, RectangleF) = New Dictionary(Of Char, RectangleF)(64)
        Private ReadOnly bmp As Bitmap
        Private ReadOnly gfx As Drawing.Graphics
        ...
        Private Shared ReadOnly default_string_format As StringFormat = StringFormat.GenericTypographic
...
#Region "--- Constructor ---"
        ...
        Friend Sub New(font As Font)
            If font Is Nothing Then Throw New ArgumentNullException(NameOf(font), "Argument to TextureFont constructor cannot be null.")
            Me.font = font
...
#End Region
    ...
    Friend Module GdiPlus
        Private ReadOnly internals As IGdiPlusInternals
...
#End Region
    ...
    Friend Class WinGdiPlusInternals
        ...
        Shared Sub New()
            native_graphics_property = GetType(Drawing.Graphics).GetProperty("NativeGraphics", BindingFlags.Instance Or BindingFlags.NonPublic)
...
#Region "--- IGdiPlusInternals Members ---"
        Friend Function GetNativeGraphics(graphics As Drawing.Graphics) As IntPtr Implements IGdiPlusInternals.GetNativeGraphics
...
#End Region
    ...
    Friend MustInherit Class GL1TextOutputProvider
        ...
        Private ReadOnly active_lists As Dictionary(Of Texture2D, List(Of Vector2)) = New Dictionary(Of Texture2D, List(Of Vector2))()
        Private ReadOnly inactive_lists As Queue(Of List(Of Vector2)) = New Queue(Of List(Of Vector2))()
        ...
        Private ReadOnly projection_stack As Stack(Of Matrix4) = New Stack(Of Matrix4)()
        Private ReadOnly modelview_stack As Stack(Of Matrix4) = New Stack(Of Matrix4)()
        Private ReadOnly texture_stack As Stack(Of Matrix4) = New Stack(Of Matrix4)()
        Private ReadOnly viewport_stack As Stack(Of Viewport) = New Stack(Of Viewport)()
        ...
        Private Const block_cache_capacity As Integer = 32
...
#Region "ITextOutputProvider Members"
        ...
        Friend Sub Print(block As TextBlock, color As Color, rasterizer As IGlyphRasterizer) Implements ITextOutputProvider.Print
            ...
            Else
                Using extents As TextExtents = rasterizer.MeasureText(block)
                    ' Build layout. On passe par foreach afin d'avoir un énumérateur qui supprime le CRLF
...
#End Region
    ...
    Friend NotInheritable Class GlyphCacheType(Of T As Texture2D)
        ...
        Private ReadOnly sheets As List(Of GlyphSheet(Of T)) = New List(Of GlyphSheet(Of T))()
        Private ReadOnly cached_glyphs As Dictionary(Of GlyphEQuatable, CachedGlyphInfo) = New Dictionary(Of GlyphEQuatable, CachedGlyphInfo)()
        Private disposed As Boolean
...
#Region "IGlyphCache Members"
        Friend Overrides Sub Add(glyph As GlyphEQuatable, rasterizer As IGlyphRasterizer, quality As TextQuality)
            If rasterizer Is Nothing Then Throw New ArgumentNullException(NameOf(rasterizer))

            Using bmp As Bitmap = rasterizer.Rasterize(glyph, quality)
                'bmp correspond à la texture du caractère nécéssaire à son dessin avec 0,0 au top-left
                Dim rect As Rectangle = New Rectangle(0, 0, bmp.Width, bmp.Height)
                Dim sheet As GlyphSheet(Of T)
                For Each sheet In sheets
                    If InsertGlyph(glyph, bmp, rect, sheet) Then Exit Sub
                Next
                'si y a plus de place on créer un autre conteneur
                sheet = New GlyphSheet(Of T)(SheetWidth, SheetHeight)
                sheets.Add(sheet)
...
#End Region
    ...
    Friend Structure GlyphPackable
        ...
        Friend Sub New(c As Char, f As Font, s As SizeF)
            If f Is Nothing Then Throw New ArgumentNullException(NameOf(f), "You must specify a valid font")
            characterField = c
            fontField = f
            sizeField = s
        End Sub
...
#End Region
    ...
    Friend Structure GlyphEQuatable
        ...
        Friend ReadOnly Property IsWhiteSpace As Boolean
            ...
            End Get
        End Property
        Friend ReadOnly Property IsControl As Boolean
            Get
...
#Region "Fields"
        ...
        Private ReadOnly block_cache As Dictionary(Of Integer, TextExtents) = New Dictionary(Of Integer, TextExtents)()
        Private ReadOnly graphics As Drawing.Graphics = Drawing.Graphics.FromImage(New Bitmap(1, 1))
...
#Region "IGlyphRasterizer Members"
        ...
        Friend Function Rasterize(glyph As GlyphEQuatable, quality As TextQuality) As Bitmap Implements IGlyphRasterizer.Rasterize
            ...
            Dim r2 = FindEdges(glyph_surface)
        ...
        Private Shared Sub EnsureSurfaceSize(ByRef bmp As Bitmap, ByRef gfx As Drawing.Graphics, font As Font)
        ...
        Private Shared Sub SetTextRenderingOptions(gfx As Drawing.Graphics, font As Font, quality As TextQuality)
            Select Case quality
        ...
        Private Function MeasureTextExtents(ByRef block As TextBlock, quality As TextQuality) As TextExtents
            ...
            'récupère les pointeurs sur les 3 objets systeme.drawing
            ...
            'décompose le texte à écrire en lignes
            ...
            For Each s As String In lines
                ...
                If (block.Direction And TextDirection.Vertical) = 0 Then
                    rect.Y += block.Font.Height 'passage à la ligne suivante
                Else
                    rect.X += block.Font.Height 'passage au caractère suivant
                End If
        ...
        Private Function MeasureGlyphExtents(ByRef block As TextBlock, text As String, native_graphics As IntPtr,
            ...
            While current < text.Length
                ...
                For i As Integer = 0 To num_characters - 1
                    Dim rect As RectangleF = New RectangleF()
                    status = GetRegionBounds(regions(i), native_graphics, rect)
                    Debug.Assert(status = 0, String.Format("GDI+ error: {0}", status))
                    status = DeleteRegion(regions(i))
                    Debug.Assert(status = 0, String.Format("GDI+ error: {0}", status))
...
#Region "FindEdges"
        ...
        Private Shared Function FindEdges(bmp As Bitmap) As Rectangle
        ...
        Private Shared Function FindRightEdge(Bmp As Bitmap, Scan0 As IntPtr) As Integer
        ...
        Private Shared Function FindBottomEdge(Bmp As Bitmap, Scan0 As IntPtr) As Integer
            Dim Stride As Integer = NbOctetsParPixel * Bmp.Width
        ...
        Friend Sub Clear1() Implements IGlyphRasterizer.Clear
            Throw New NotImplementedException()
        End Sub
#End Region
#End Region
    ...
    Friend Class GlyphEnumerator
        ...
        Private ReadOnly implementation As IEnumerator(Of Char)

        Friend Sub New(text As String, font As Font)
            If Equals(text, Nothing) Then Throw New ArgumentNullException(NameOf(text))
            If font Is Nothing Then Throw New ArgumentNullException(NameOf(font))
            Me.font = font
...
#End Region
    ...
    Friend Class PoolableTextExtents
        ...
        Property Owner As ObjectPool(Of PoolableTextExtents) Implements IPoolableType(Of PoolableTextExtents).Owner
            Get
                Return ownerField
            End Get
        ...
        Sub OnAcquire() Implements IPoolable.OnAcquire
            Clear()
        End Sub

        Sub OnRelease() Implements IPoolable.OnRelease
        End Sub
    End Class
...
#End Region
    ...
    Friend Interface ITextOutputProvider
        ...
        Sub Print(block As TextBlock, color As Color, rasterizer As IGlyphRasterizer)
        Sub Clear()
        Sub Begin()
        Sub [End]()
    End Interface
